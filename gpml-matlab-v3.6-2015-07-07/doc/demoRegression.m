...
%adding the test inputs z as the final argument 
%to compute the predictions
%GP returns the mean and the variance at the test locations
...
% we don't typically know values of the hyperparameters
% we will assume a particular structure and learn suitable hyperparameters
%our guess is that squared exponential covariance function convSEiso must
%be suitable 
covfunc = @covSEiso; hyp2.cov = [0; 0]; hyp2.lik = log(0.1);
disp('hyp2 = minimize(hyp2, @gp, -100, @infExact, [], covfunc, likfunc, x, y)')

%optimizing the hyperparameters by minimizing
%the negative log likelihood wrt to the hyperparamters
...
%we want to add more flexibility in the interpretation
%We want the confidence region for the underlying function
%
disp(' ')
